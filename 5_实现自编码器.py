...
# 自编码，输入层的神经元数等于输出层的神经元数
# 其目标是使用稀疏的一些高阶特征重新组合来重构自己

# 初始化权重，让权值满足0均值
# fan_in 输入节点的数目，fan_out 输出节点的数目
...
class AdditiveGaussianAutoencoder(object):
    def __init__(self, n_input, n_hidden, transfer_function=tf.nn.softplus,
        ...
        self.noisex = self.x + scale * \
            tf.random_normal((n_input,))  # 加入噪声的输入图像
        # 下面是给输入的数据加入了噪声
        self.hidden = self.transfer(tf.add(tf.matmul(
            self.noisex, self.weights['w1']), self.weights['b1']))
        # 上一行是隐含层
        # 输出层
    ...
    def _initalize_weights(self):
        ...
        all_weights['w2'] = tf.Variable(
            tf.zeros([self.n_hidden, self.n_input], dtype=tf.float32))  # 输出神经元数和输入一样
